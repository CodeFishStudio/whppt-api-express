...
module.exports = (context, mod, handlerName, params, req) => {
    return Promise.resolve().then(() => {
        ...
        const _context = Object.assign(Object.assign({}, context), { createEvent });
        if (!callHandler.authorise) {
            return Promise.resolve()
                ...
                return Promise.reject({
                    ...
                    error: new ModuleExecError(err && err.status, `Error executing Module: ${mod}/${handlerName}, Error - ${err.message}`, err.message || err.error || err),
                });
            });
        }
        return Promise.resolve()
            ...
            return Promise.reject({
                status: 403,
                error: new AuthError(`Not Authorised to call Module: ${mod}/${handlerName}`, err),
            });
        })
            .then(() => callHandler.exec(_context, params, req).catch(err => {
            return Promise.reject({
                status: (err && err.status) || 500,
                error: new ModuleExecError(err && err.status, `Error executing Module: ${mod}/${handlerName} - ${err.message}`, err),
            });
        }));
    });
};
...
//# sourceMappingURL=callModule.js.map