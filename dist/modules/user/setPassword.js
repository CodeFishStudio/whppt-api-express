...
module.exports = {
    exec({ $security, $mongo: { $db, $save }, $hosting }, { username, email, password, token }) {
        ...
        return $hosting.then(({ security }) => {
            return $db
                ...
                const tokenToCompare = crypto
                    ...
                    .digest('hex');
                const tokenMatches = matchingUser.passwordResetToken.token === token &&
                ...
                if (!tokenMatches)
                    throw new Error('Failed to validate the provided token.');
                if (tokenExpired)
