...
module.exports = ({ $mongo: { $db }, $pageTypes, $fullUrl }, { page, size, slug, freq, pageType, priority, lastModTo, lastModFrom, domainId }) => {
    ...
    if (slug)
        filters.slug = { $regex: slug === '/' ? '' : slug, $options: 'i' };
    if (pageType)
        filters.pageType = pageType;
    if (priority)
        filters.priority = Number(priority);
    if (lastModFrom)
        filters.updatedAt = { $gte: new Date(lastModFrom) };
    if (lastModTo)
        filters.updatedAt = { $lte: new Date(lastModTo) };
    if (domainId)
        filters.domainId = domainId;
    return Promise.all(map(collections, collection => {
        return $db
            ...
            .toArray()
            .then(pages => {
            return map(pages, _page => (Object.assign(Object.assign({}, _page), { url: $fullUrl(_page.slug) })));
        });
    })).then(sitemaps => {
        const fullSitemap = flatten(sitemaps);
        if (!page && !size)
            return { sitemap: fullSitemap, total: fullSitemap.length };
        return {
            sitemap: take(drop(orderBy(fullSitemap, ['slug'], ['asc']), size * (page - 1)), size),
